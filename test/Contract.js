...
describe("Contract", () => {
  // ANDRE:
  // haria un beforeEach deployando el contrato antes de cada test
  // https://github.com/andrebrener/dp_NFT_drop/blob/main/smart_contracts/test/DPDrop.js
  ...
  describe("Packs", () => {
    ...
    it("Should fail to buy more than the available packs", async () => {
      // ANDRE: me parece mejor usar directo el fail que el try catch
      // Algo asi:
  ...
  describe("Trades", () => {
    ...
    it("Should fail if starting a trade with invalid type", async () => {
      try {
        ...
        await pixelCup
          .connect(userA)
          .startTrade(stickerReq, Number(reqCountry), 4, 0);
      } catch (err) {
    ...
    it("Should make a trade for a specific shirt number", async () => {
      ...
      try {
        await pixelCup
          .connect(userA)
          .completeTrade(tradeIndex, stickerReq.number + 1);
      } catch (e) {
      ...
      const ownerTradesAfterComplete = await pixelCup.ownerTrades(
        userB.address
      );
